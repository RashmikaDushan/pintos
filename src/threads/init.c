...
static void read_line(char *input_line);
...
int pintos_init(void)
{
  ...
  char input_line[LOADER_ARGS_LEN / 2 + 1];
  while (true)
  {
    // Read the line
    read_line(input_line);
    if (!strcmp(input_line, "whoami"))
    {
      printf("Name:- Balasooriya K.B.R.D.     Index:- 210057R \n"); // Display my name alongside my index number
    }
    else if (!strcmp(input_line, "time")) // Display the number of seconds passed since Unix epoch
    {
      time_t time = rtc_get_time();
      printf("Elapsed time: %lu seconds\n", time);
    }
    else if (!strcmp(input_line, "ram")) // Display the amount of RAM available for the OS
    {
      printf("Available RAM is: %'" PRIu32 " kB RAM\n", init_ram_pages * PGSIZE / 1024);
    }
    else if (!strcmp(input_line, "thread")) // Display thread statistics
    {
      thread_print_stats();
    }
    else if (!strcmp(input_line, "priority")) // Display the thread priority of the current thread
    {
      printf("Priority of the current thread %d\n", thread_get_priority());
    }

    else if (!strcmp(input_line, "shutdown")) // Pintos OS will shutdown and exit the qemu emulator
    {
      shutdown_power_off();
    }
    else if (!strcmp(input_line, "exit")) // Exit the interactive shell
    {
...
/* Breaks the input into words and returns them as
   a character array. */
static void read_line(char *input_line)
{
  int input_len = LOADER_ARGS_LEN / 2 + 1;
  int index = 0;
  char letter;
  ...
  while (index < input_len + 1) // Read the line character by character
  {
    letter = input_getc();
    if (letter == '\r')
    {
